function FEM_diffusion_P1_PLA(N,mesh_np,mm,l)
 
% Finite element method with Lagrange P^1 basis functions and parrallel
...
[Coord,Elem,Nb,Db] = initial_mesh(mesh_np);     
...
nd = size(Coord,1);             % number of nodes
...
tic
assembly_rows_g = reshape(repmat(Elem(:,1:3)',3,1),9*n_elem,1);
assembly_cols_g = reshape(repmat(reshape(Elem(:,1:3)',1,[]),3,1),[],1);


m_stima = asmbl_mat_gen(elem_st,mm,l,n_elem);


A = sparse(assembly_rows_g,assembly_cols_g,m_stima,nd,nd);
toc

for j=1:n_elem
    b(Elem(j,:))= b(Elem(j,:)) + get_integral(2,Coord(Elem(j,1:3),:),deg);
end   
...
if (~isempty(Nb))
   for j=1:size(Nb,1)
       b(Nb(j,1:deg+1))= b(Nb(j,1:deg+1)) +...
                 int_gaussian(2,Coord,Elem,Nb(j,:),deg,mm);   
   end
end
